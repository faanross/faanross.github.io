...
In the next 3 lessons we'll create our complete HTTPS logic, including the server (this lesson), agent (Lesson 3), and run loop (Lesson 4). Thereafter we'll do the exact same thing for DNS.
...
- `internals/server/server_https.go` - Complete HTTPS server implementation
- `HTTPSServer` struct representing our server instance
- `HTTPSResponse` struct for JSON responses
- `NewHTTPSServer()` constructor function
- `Start()` and `Stop()` methods to satisfy the Server interface
- `RootHandler()` handler function for the root endpoint
...
Let's create a new file in `internals/server/server_https.go` that'll house all the logic for our HTTPS server.
...
For now just implement one field related to ability to change or not we will add more later!
...
This is going to be marshalled to JSON before going on the wire, notice how we use JSON struct tags to control field naming during marshaling/unmarshaling.
...
So back in `./internals/server/models.go`, in the `NewServer` function in the case for `https` we no longer return an error, instead we go and call our `NewHTTPSServer` constructor.
...
We create the config directly in Go code using a struct literal. This is simpler and keeps all configuration visible in one place.

Thereafter we call our `server.NewServer()` factory function, which, in the case that our stated protocol is `https`, will return an instantiated `HTTPSServer` struct.
...
```shell
go run ./cmd/server
2025/08/21 15:49:59 Starting https server on 0.0.0.0:8443
```
...
```shell
go run ./cmd/server
2025/08/14 08:11:21 Starting https server on 0.0.0.0:8443
